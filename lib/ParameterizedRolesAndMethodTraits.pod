...
Parameterized Roles and Method Traits - Discussion on MooseX::Declare advanced composition features

=head1 INTRODUCTION

This is a discussion of some advanced features of L<Moose> and L<MooseX::Declare>.  If you are new to Perl or Moose I recommend the following tutorials and web resources to get you up to speed.  These should be your first stop if you are new to L<Moose> or never heard of roles.

=over4

=item Learning Perl

See the web site L<Learning Perl:http://learn.perl.org>
...
The L<Moose Homepage:http://moose.perl.org> has a good collection of links to blogs, articles and presentations.
...
This gives you a more modern class and method declaration syntax which lets you dispense with all the boilerplate validation and mucking around with "@_".  You get L<Moose> type constraint checking in method signatures for free.  Additionally this looks much more like what a modern programmer expects to see when creating classes, which should make it easier for people coming from other languages to get excited about Perl.  The only (current) downsides are a performance penalty and sometimes if you have syntax errors the error messages can be a little cryptic.  Both issues are a work in progress but if they really bother you you can always drop down to 'classic' Moose and use L<Method::Signatures::Simple> to get the shiny 'method' keyword (although you loose the method body signature type constraint checking). 
...
This would create a role that decorates a method of choice with some additional text prepended to to start of the method retunr.  You might use it like:
...
Best practices continue to evolve and there's plenty of room open for discussion, examples and contributions.
